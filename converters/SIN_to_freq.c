...
void do_calculate(GtkWidget *calculate, gpointer data) {
    int oct_band = atoi((char *)gtk_entry_get_text(GTK_ENTRY(number1)));
    int note = atoi((char *)gtk_entry_get_text(GTK_ENTRY(number2)));

    // See cuppajoeman.com/music for meaning behind conversion and naming

    char buffer[32];
    snprintf(buffer, sizeof(buffer), "result: %lf", 440 * pow(2.0, (note - 9)/12.0) * pow(2.0, oct_band - 4));
