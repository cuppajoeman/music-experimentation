...
TRUE_WIDTH = PIXEL_SCALE*(WIDTH + 3 * PADDING_X)
TRUE_HEIGHT = PIXEL_SCALE*(HEIGHT + 2 * PADDING_Y)

NUM_SEMITONES = 12
NUM_DEGREES = 7

X_UNIT = WIDTH/(NUM_DEGREES-1)
Y_UNIT = HEIGHT/(NUM_SEMITONES-1)
...
def create_table_for_patterns(filename, patterns, printable=False):
    ...
    for j in range(num_patterns):
        ...
        for i in range(NUM_DEGREES + 1):
            y_pos = HEIGHT/(NUM_DEGREES - 1) * i
            ctx.move_to(0, y_pos)
        ...
        full_table = [top_row , gen_table_from_pattern(pattern)]
...
def gen_table_from_pattern(pattern):
    table = []
    for i in range(len(pattern)):
        table.extend(sum_pat(scales['maj'], i))
    return table
...
def draw_table(table, ctx, printable=False):
    ...
    for x in range(len(table)):
        # Iterate down a column (down the rows)
